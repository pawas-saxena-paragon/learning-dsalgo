// https://youtu.be/4WmTRFZilj8?si=LQ3H3gddYhar6O1A
...
export function search(nums: number[], target: number): number {
  const rotationPivot: number = getRotationPivotIndex(nums);
 ...
 */
export function getRotationPivotIndex(nums: number[]): number {
  let start = 0;
